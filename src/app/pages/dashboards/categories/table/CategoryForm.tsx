import type { ChangeEvent, FormEvent } from "react";

import { Button, Card, Input, Switch } from "@/components/ui";
import type { CategoryItem } from "@/app/services/endpoints/categories";

interface CategoryFormState {
  title: string;
  slug: string;
  description: string;
  imageUrl: string;
  image?: File;
  parentId: string;
  isActive: boolean;
  sortOrder: string;
}

interface CategoryFormProps {
  form: CategoryFormState;
  treeItems: CategoryItem[];
  editingItem: CategoryItem | null;
  submitting: boolean;
  onChange: (
    key: keyof CategoryFormState,
    value: string | boolean | File | undefined,
  ) => void;
  onSubmit: () => void;
  onCancel: () => void;
}

export function CategoryForm({
  form,
  treeItems,
  editingItem,
  submitting,
  onChange,
  onSubmit,
  onCancel,
}: CategoryFormProps) {
  const parentOptions = treeItems.filter((item) => item.id !== editingItem?.id);

  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    onSubmit();
  };

  return (
    <Card className="p-4 sm:p-5">
      <form onSubmit={handleSubmit}>
        <div className="mb-4 flex items-center justify-between">
          <h2 className="dark:text-dark-100 text-base font-semibold text-gray-800">
            {editingItem ? "ویرایش دسته‌بندی" : "ایجاد دسته‌بندی"}
          </h2>
        </div>

        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="flex flex-col gap-2">
            <label className="dark:text-dark-100 text-sm font-medium text-gray-700">
              عنوان
            </label>
            <Input
              placeholder="عنوان دسته‌بندی را وارد کنید"
              value={form.title}
              disabled={submitting}
              onChange={(e: ChangeEvent<HTMLInputElement>) =>
                onChange("title", e.target.value)
              }
            />
          </div>

          <div className="flex flex-col gap-2">
            <label className="dark:text-dark-100 text-sm font-medium text-gray-700">
              ترتیب نمایش
            </label>
            <Input
              type="number"
              placeholder="0"
              value={form.sortOrder}
              disabled={submitting}
              onChange={(e: ChangeEvent<HTMLInputElement>) =>
                onChange("sortOrder", e.target.value)
              }
            />
          </div>

          <div className="flex flex-col gap-2">
            <label className="dark:text-dark-100 text-sm font-medium text-gray-700">
              دسته‌بندی والد
            </label>
            <select
              className="dark:bg-dark-900 dark:border-dark-600 h-10 rounded-lg border border-gray-300 px-3"
              value={form.parentId}
              disabled={submitting}
              onChange={(e) => onChange("parentId", e.target.value)}
            >
              <option value="">بدون والد</option>
              {parentOptions.map((item) => (
                <option key={item.id} value={item.id}>
                  {item.title}
                </option>
              ))}
            </select>
          </div>

          <div className="flex flex-col gap-2 md:col-span-2">
            <label className="dark:text-dark-100 text-sm font-medium text-gray-700">
              توضیحات
            </label>
            <Input
              placeholder="توضیحات دسته‌بندی"
              value={form.description}
              disabled={submitting}
              onChange={(e: ChangeEvent<HTMLInputElement>) =>
                onChange("description", e.target.value)
              }
            />
          </div>

          <div className="flex items-center gap-3">
            <span className="dark:text-dark-100 text-sm font-medium text-gray-700">
              فعال
            </span>
            <Switch
              checked={form.isActive}
              disabled={submitting}
              onChange={(e: ChangeEvent<HTMLInputElement>) =>
                onChange("isActive", e.target.checked)
              }
            />
          </div>
        </div>

        <div className="mt-5 flex flex-wrap gap-2">
          <Button type="submit" disabled={submitting}>
            {submitting
              ? "در حال ذخیره..."
              : editingItem
                ? "بروزرسانی دسته‌بندی"
                : "ایجاد دسته‌بندی"}
          </Button>

          <Button
            type="button"
            variant="outlined"
            onClick={onCancel}
            disabled={submitting}
          >
            انصراف
          </Button>
        </div>
      </form>
    </Card>
  );
}

export default CategoryForm;
